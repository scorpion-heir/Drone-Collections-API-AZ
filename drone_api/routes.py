...
def signup():
    form = UserLoginForm()
#try to get info from the form
    try:
        if request.method == 'POST' and form.validate_on_submit():
            email = form.email.data #grab data
            password = form.password.data 
            print(email, password)
            #take data sent to model
            user = User(email, password = password)

            db.session.add(user) #similar to add to github
            db.session.commit() #similar to commit/save to github 
...
# CRUD: retrieve all drones endpoint
@app.route('/drones', methods = ['GET'])
@token_required
def get_drones(current_user_token):
    try: #adding ownership to ??
...
# CRUD: retrieve one drone endpoint 
@app.route('/drones/<id>', methods = ['GET'])
@token_required
def get_drone(current_user_token, id): #search drone by id
    try: 
        owner, current_user_token = verify_owner(current_user_token)
    except:
        bad_res = verify_owner(current_user_token)
...
def update_drone(current_user_token, id):
    try: 
        ...
        bad_res = verify_owner(current_user_token)
        return bad_res
    #grab owner, if receive onwer successfully 
    drone = Drone.query.get(id) # grab a drone instance
    #update the drone instance
    drone.name = request.json['name']  
    drone.price = request.json['price']
    drone.model = request.json['model']

    db.session.commit()
    response = drone_schema.dump(drone) #ask for a response of new info
    return jsonify(response)

# # CRUD: delete drone endpoint 
...
def delete_drone(current_user_token, id):
    try: #happy path
        ...
        bad_res = verify_owner(current_user_token)
        return bad_res
    drone = Drone.query.get(id) #id for drone 
